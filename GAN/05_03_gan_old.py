...
class dcgan():
    def __init__(self,data,region,building):
        self.region = region
    ...
    def train(self,steps,batch_size,save_interval,inPath,outPath,num_img,num_train):
        #labels
        ...
        #other regions
        other_1 = np.load(inPath + '02_img_' + sys.argv[2] + '_' + self.building + '.npy')
        other_2 = np.load(inPath + '02_img_' + sys.argv[3] + '_' + self.building + '.npy')

        try:
            for t in range(num_train):
                print('\nrun %d:'%(t+1),end=' ')
                outPath_current = outPath + 'run_%d'%(t+1)
                os.mkdir(outPath_current)
                outPath_current += '/'
                for i in range(steps):
                    ...
                    (d_loss[i],d_acc[i]) = np.add(0.5 * d_loss_fake,0.5 * np.array(d_loss_real))
                    ...
                    if (i + 1) % save_interval == 0:
                        self.save_everything(i + 1, outPath_current, d_loss, d_acc, g_loss, num_img)

        except KeyboardInterrupt as k:
          with open(outPath_current + 'stopped', 'w') as f:
            f.write('Script was stopped on the %d. step at %s' % (i, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
          self.save_everything(i, outPath_current, d_loss, d_acc, g_loss, num_img)

        except Exception as e:
            print('exception')
            with open(outPath_current + 'exception', 'w') as f:
              f.write('Script was stopped on the %d. step at %s' % (i, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
              f.write('\n')
    ...
    def save_models(self, i, path):
      ...
      self.generator.save(path + 'gen_%d' % i)
...
building = sys.argv[4]
inPath = '/home/nagfa5/GAN/05_stocha_c/' + building + '/'
outPath = '/home/nagfa5/GAN/05_stocha_d/old_loss/' + building + '/'
region = sys.argv[1]
dataFile = '02_img_'+region+'_'+building+'.npy'
batch_size = 50
save_interval = 50
train_steps = 2000
num_trains = 100
...
GAN = dcgan(data,region,building)
GAN.train(train_steps,batch_size,save_interval,inPath,outPath,num_img,num_trains)
