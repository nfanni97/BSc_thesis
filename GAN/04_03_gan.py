...
class dcgan():
    ...
    def train(self,steps,batch_size,save_interval,path,num_img):
        ...
        #other regions
        other_1 = np.load(path + '02_data_' + sys.argv[2] + '.npy')
        other_2 = np.load(path + '02_data_' + sys.argv[3] + '.npy')
        ...
        try:
          ...
          with open(path + self.region + '_stopped', 'w') as f:
          ...
          self.save_everything(i, path, d_loss, d_acc, g_loss, num_img)
            ...
            print('exception')
            with open(path + self.region + '_exception', 'w') as f:
              f.write('Script was stopped on the %d. step at %s' % (i, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
              f.write('\n')
    ...
    def save_results(self, i, path, d_loss, d_acc, g_loss):
      np.savetxt(path + self.region + '_d_loss', d_loss, delimiter='\n')
      np.savetxt(path + self.region + '_d_acc', d_acc, delimiter='\n')
      np.savetxt(path + self.region + '_g_loss', g_loss, delimiter='\n')

    def save_models(self, i, path):
      self.gan.save(path + 'models/' + self.region + '/gan_%d' % i)
      self.discriminator.save(path + 'models/' + self.region + '/disc_%d' % i)
      self.generator.save(path + 'models/' + self.region + '/gen_%d' % i)
    ...
    def save_images(self, i, path, img_num):
      ...
      np.save(path + 'generated/' + self.region + '/%d' % i, generated)
...
outPath = '/home/nagfa5/GAN/04_groups/'
region = sys.argv[1]
...
GAN.train(steps,batch_size,save_interval,outPath,num_img)
